// Build: defaultdb-no-sw-v3-daily-records
...
const els = {
  ...
  dailyTotalCount: document.getElementById("dailyTotalCount"),
  dailyWithin15Count: document.getElementById("dailyWithin15Count"),
  daily15to25Count: document.getElementById("daily15to25Count"),
  daily25to30Count: document.getElementById("daily25to30Count"),
  dailyOver30Count: document.getElementById("dailyOver30Count"),
...
function attachCardEvents(card, order) {
  ...
  if (PAGE_MODE !== "hostess") {
    completeBtn?.addEventListener("click", async () => {
      const liveOrder = state.orderMap.get(order.id) || order;
      await updateOrder(order.id, {
        completed: true,
        completedAtMs: Date.now(),
        completionDurationMs: getElapsedMs(liveOrder),
        updatedAt: serverTimestamp(),
      });
    });
  }
...
function updateStats() {
  if (els.activeCount && els.overdueCount && els.completedCount) {
    const visibleOrders = getVisibleOrders();
...
function updateDailyRecordStats() {
  if (!els.dailyTotalCount && !els.dailyCompletedCount) return;

  const todayOrders = state.orders.filter((order) => isTodayOrder(order));
  let completedToday = 0;
  let within15Count = 0;
  let range15to25Count = 0;
  let range25to30Count = 0;
  let over30Count = 0;

  for (const order of todayOrders) {
    if (!order.completed) continue;
    completedToday += 1;
    const durationMs = getCompletionDurationMs(order);
    if (durationMs <= 15 * 60 * 1000) {
      within15Count += 1;
    } else if (durationMs <= 25 * 60 * 1000) {
      range15to25Count += 1;
  ...
  if (els.recordDateLabel) {
    els.recordDateLabel.textContent = `อัปเดตตามวันที่ ${formatThaiShortDate(Date.now())}`;
  }
  if (els.dailyTotalCount) els.dailyTotalCount.textContent = String(todayOrders.length);
  if (els.dailyWithin15Count) els.dailyWithin15Count.textContent = String(within15Count);
  if (els.daily15to25Count) els.daily15to25Count.textContent = String(range15to25Count);
  if (els.daily25to30Count) els.daily25to30Count.textContent = String(range25to30Count);
  if (els.dailyOver30Count) els.dailyOver30Count.textContent = String(over30Count);
  if (els.dailyCompletedCount) els.dailyCompletedCount.textContent = String(completedToday);
}

function isTodayOrder(order) {
  return getLocalDayKey(order?.createdAtMs) === getLocalDayKey(Date.now());
}
...
function getCompletionDurationMs(order) {
  const explicit = Number(order?.completionDurationMs || 0);
  if (explicit > 0) return explicit;

  const createdAtMs = Number(order?.createdAtMs || 0);
  const completedAtMs = getCompletedAtMs(order);
  if (createdAtMs > 0 && completedAtMs > 0 && completedAtMs >= createdAtMs) {
    return completedAtMs - createdAtMs;
  }

  return Math.max(0, getElapsedMs(order));
}

function getCompletedAtMs(order) {
  const direct = Number(order?.completedAtMs || 0);
  if (direct > 0) return direct;
  const updatedAtMs = order?.updatedAt?.toMillis?.();
  return Number(updatedAtMs || 0);
}

function formatThaiShortDate(value) {
  try {
    return new Date(Number(value || Date.now())).toLocaleDateString("th-TH", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
